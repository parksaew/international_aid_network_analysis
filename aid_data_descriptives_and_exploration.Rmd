...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      dateRangeInput(inputId = "yearRange",
                     label = "Year range:",
                     start = "1991-01-01",
                     end = "2016-12-31",
                     min = "1991-01-01",
...
server = function(input, output) {
  ...
  output$aidTrends <- renderPlot({ #this takes forever to load
    ...
    if (input$recipientRegion != "All regions") { #applying region filters
      econ_dash_dyadic <- econ_dash_dyadic %>%
    ...
    aidTrendsGG <- ggplot(econ_dash_dyadic) +
      geom_line(aes(x = year,
                    y = value,
                    group = dyad_name, #group splits the data like color or fill but does not apply additional visual differentiation
                    color = r_region)) +
  ...
  height = 200) #might be better to set height in the ui side
