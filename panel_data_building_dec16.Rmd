...
ucdp_clean <- ucdp_2014 %>% 
  ...
  rename(cowcode = gwno,
         ...
         new_conflict = newconflictinyearv414,
         onset_2yr = onset2v414) %>%
  group_by(cowcode) %>% 
  arrange(cowcode,
          year) %>% #checked that no years were missing using the custom function check_missing_sequence
...
ucdp_onset_2014 <- slide(ucdp_2014, Var = "incidencev414", GroupVar = "gwno", slideBy = -1, reminder = F)
...
ucdp_onset_2014_pyears <- btscs(ucdp_onset_2014, "incidencev414", "year", "gwno") ## pickup HERE *****
