...
crs_panel_clean <- crspanel %>%
  ...
  mutate(aid_disburse = aid_disburse/deflator, #converting to 2016 constant USD
         aid_commit = aid_commit/deflator) %>%
  group_by(ccode1, ccode2, year, aid_type) %>%
  summarise(aid_disburse = sum(aid_disburse, na.rm = T),
            aid_commit = sum(aid_commit, na.rm = T)) %>%
  pivot_wider(names_from = aid_type, #spread data s.t each combination of aid type by disbursement/commitment has a colum
              values_from = c(aid_disburse,
                              aid_commit),
              names_sep = "_") %>%
  replace(is.na(.),
          0)
  
#Todo (after May 20): figure out why crs_panel_clean has fewer values than crsall_sp  




####### (OLD) Clean the CRS data ########
