...
crs_ip_country_controls <- crs_ip %>%
  left_join(original_controls %>% #merge the data for recipient countries
            ...
            by = c("ccode2" = "cowcode", #ccode2 is for recipient countries
                              "year")) %>%
  left_join(original_controls %>% #merge the data for donor countries. PICKUP from here
              select(cowcode,
                     year,
  ...
  mutate_at(vars(contains("_corrected")), #lags to measure the effect of previous aid amounts or FD (NEEDS VALIDATION)
            list(`1` = ~lag(.),
                 ...
                 `5` = ~lag(.,
                            n = 5))) %>%
  mutate_at(vars(matches("^(d_|r_)")), #lags to measure the effect of previous year variables or FD (NEEDS VALIDATION)
...
desta_clean <- rbind(desta_simple,
  ...
  group_by(ccode1, 
           ccode2) %>%
  complete(year = 1948:2017) %>% #creates rows for "missing" years - we assume that there were no PTAs in the missing years
...
trade_year_uncorrected <- trade %>%
  select(exporting_ccode_imf = Country.Code,
         ...
         year = Time.Period,
         export = Goods..Value.of.Exports..Free.on.board..FOB...US.Dollars..TXG_FOB_USD.) %>% #Q: do we need to correct for inflation?
...
trade_missing_year_list <- lapply(with(trade_clean %>%
...
trade_clean <- trade_year_uncorrected %>%
  full_join(trade_missing_indices,
            by = c("exporting_ccode",
                   "importing_ccode",
                   "year")) %>%
  mutate(export_log = log(export + 1)) %>%
  group_by(exporting_ccode,
           importing_ccode) %>%
  arrange(exporting_ccode,
          importing_ccode,
          year) %>%
  mutate_at(vars(matches("^export$|export_")),
            list(`1` = lag))
...
mergedata3 <- left_join(mergedata2,
  ...
  #mutate(ptacount = ifelse(is.na(ptacount), 0, ptacount), #added up duplicates already ## ALREADY DONE IN DESTA SECTION
  ...
  left_join(select(immigration,
            ...
            by = c("ccode1",
                   "ccode2",
                   "year")) %>% #adding a 5-year rolling sum of naturalization #*** June 2020 use rollsum from zoo or another known pack
  # mutate(naturalization_roll = roll_sum(naturalization, 5, fill = NA, align = "right"), #right means x[i-2] + x[i-1] + x[i] (if n = 3) so that it doesn't look to the future
  ...
  #        log_immigration_roll_1 = lag(log_immigration_roll, order_by = year)) %>% #*** already done in immigration section *****
  ...
  # mutate(log_rtod_export = log(rtod_export+1), # already done in IMF trade section (June 2020) but need to add rtod or dtor
