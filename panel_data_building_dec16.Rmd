...
#crspanel <- read.csv("./crs/crspanel.csv", stringsAsFactors = F) #only run when restarting environment
...
crs_panel_clean <- crspanel %>%
  ...
  left_join(select(deflators, #add the deflators to use for conversion later
                   -country_name), # **** CHECK HOW MANY are not matched (only 29 currencies here)**** may need new deflators
            by = c("Year" = "year", 
                   "cow_donor" = "cow_code")) %>%
...
#We will merge several country-year datasets together. First, we create a skeleton of just the index variables
